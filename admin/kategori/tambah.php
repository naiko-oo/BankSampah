...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_kategori)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO categories (nama_kategori, harga_per_kg, deskripsi) VALUES (:nama, :harga, :deskripsi)");
            $stmt->execute([
                ':nama'      => $nama_kategori,
                ':harga'     => $harga_per_kg,
                ':deskripsi' => $deskripsi
            ...
            set_flash('success', "Kategori <strong>" . htmlspecialchars($nama_kategori) . "</strong> berhasil ditambahkan.");
...
$pageTitle = 'Tambah Kategori Sampah';
        ...
        <div class="card border-0 shadow-sm rounded-3 p-4">
            ...
            <form method="POST">
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="nama_kategori" name="nama_kategori" placeholder="Contoh: Botol Plastik PET Bersih" required value="<?= htmlspecialchars($_POST['nama_kategori'] ?? ''); ?>">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="100" min="100" class="form-control" id="harga_per_kg" name="harga_per_kg" placeholder="3000" required value="<?= htmlspecialchars($_POST['harga_per_kg'] ?? ''); ?>">
                        <span class="input-group-text">/ kg</span>
                    </div>
                    <div class="form-text">Nilai rupiah yang akan diterima nasabah untuk setiap 1 kg sampah.</div>
                </div>

                <div class="mb-4">
                    <label for="deskripsi" class="form-label fw-semibold">Deskripsi / Syarat Penyetoran</label>
                    <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3" placeholder="Contoh: Bersih, kering, label dilepas"><?= htmlspecialchars($_POST['deskripsi'] ?? ''); ?></textarea>
                </div>
